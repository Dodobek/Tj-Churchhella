    ...
    if photo:
        try:
            if str(photo).startswith("http"):
                ...
                local_path = photo if os.path.isabs(photo) else os.path.join(BASE_DIR, photo)
                with open(local_path, "rb") as f:
                    await context.bot.send_photo(chat_id, photo=f, caption=caption, reply_markup=reply_markup, parse_mode=ParseMode.HTML)
                return
        except Exception:
            logger.warning(
                "Не удалось отправить фото. photo=%r base_dir=%s — отправляю без картинки.",
                photo, BASE_DIR, exc_info=True,
            )
...
async def send_qty_picker_card(
    ...
    rows = [qty_buttons[i : i + 2] for i in range(0, len(qty_buttons), 2)]
